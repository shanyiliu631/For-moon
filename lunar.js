...
class Lunar {
    // 农历数据表 - 每个数据代表一年的农历信息
    // 数据格式: [闰月月份(4位) | 12个月的大小月(12位) | 闰月是否为大月(1位) | 保留(3位)]
    static lunarInfo = [
    ...
    static solar2Lunar(year, month, day) {
        ...
        const base = new Date(1900, 0, 30);  // 1900-01-31
        const target = new Date(year, month - 1, day);
        
        // 计算天数差（包括起点）
        const offset = Math.floor((target - base) / (24 * 3600 * 1000));
        ...
        for (let i = 1; i <= 13; i++) {
            ...
            if (i > leapMonth && leapMonth > 0) {
                // 如果已经过了闰月，月份要往前推一位
                daysInMonth = this.getDaysInMonth(lunarYear, i - 1, false);
            } else {
                daysInMonth = this.getDaysInMonth(lunarYear, i, isLeap);
            }
        ...
        return {
            ...
            toString: function() {
                ...
                if (leapMonth > 0 && this.month > leapMonth && this.month < leapMonth + 2) {
                    // 闰月显示
                    monthStr = '闰' + months[this.month - 2];
                } else {
                    let m = this.month;
                    if (leapMonth > 0 && this.month > leapMonth + 1) {
                        m = this.month - 1;
                    }
                    monthStr = months[m - 1] || '正';
                }
    ...
    static getDaysInMonth(year, month, isLeap) {
        ...
        if (isLeap) {
            return (info >> 0) & 0x01 ? 30 : 29;
        }
        
        // 从高位到低位分别代表1月到12月
        const bit = 16 - month;
        return (info >> bit) & 0x01 ? 30 : 29;
    }
}
